...
import { FileCode, Download, RotateCcw, Loader2, Shield, Zap, Check, ArrowLeftRight } from "lucide-react";
...
// Traced mode settings tuned for quality
...
// Maximum dimension used for tracing (upscale factor restores original size in SVG)
...
async function convertPixelPerfect(file: File): Promise<Result> {
  ...
  // SVG that embeds the raster image — 100% pixel-accurate, infinitely scalable container
  const svgString = [
  ...
  return {
    name: file.name.replace(/\.(png|jpe?g|webp|gif|bmp)$/i, ".svg"),
...
async function convertTraced(file: File, mode: "high-detail" | "flat" | "monochrome"): Promise<Result> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // Scale down large images before tracing for speed — SVG scale option restores size
      ...
      const svgScale = 1 / downFactor; // upsample path coordinates back to native size
...
async function convertFile(file: File, mode: Mode): Promise<Result> {
  if (mode === "pixel-perfect") return convertPixelPerfect(file);
  return convertTraced(file, mode as "high-detail" | "flat" | "monochrome");
}
...
function CompareSlider({ original, svg }: { original: string; svg: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Labels */}
      <span className="absolute top-2 left-2 text-[10px] font-semibold bg-black/60 text-white px-1.5 py-0.5 rounded pointer-events-none">
        Original
      </span>
      <span className="absolute top-2 right-2 text-[10px] font-semibold bg-black/60 text-white px-1.5 py-0.5 rounded pointer-events-none">
        SVG
      </span>
    </div>
  );
...
export default function PngToSvgPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    for (const f of files) {
      try { ok.push(await convertFile(f, mode)); }
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "png-to-svg.zip"; a.click();
    ...
    <div className="min-h-[calc(100vh-8rem)]">
      <div className="max-w-[1200px] mx-auto px-5 md:px-6 lg:px-8 pt-8 sm:pt-12">
        <ToolHero
          ...
          description="Convert raster images to scalable SVG — pixel-perfect embedding or full-color vector tracing. Free, client-side, no upload required."
        ...
        {!done ? (
          <div className="glass-panel rounded-[16px] p-6 sm:p-8 space-y-6">
            <FileUpload
              ...
              description="or click to browse — PNG, JPG, WebP, GIF, or BMP"
            />
              ...
              <div className="space-y-5 animate-fade-in-up">
                    ...
                    {(Object.entries(MODES) as [Mode, typeof MODES[Mode]][]).map(([key, m]) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-semibold ${mode === key ? "text-primary" : "text-foreground"}`}>
                            {m.label}
                          </span>
                          <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${m.badgeColor}`}>
                            {m.badge}
                          </span>
                          {m.recommended && (
                ...
                {mode === "pixel-perfect" ? (
                  <div className="rounded-lg bg-green-50 border border-green-200 p-3 text-xs text-green-800 leading-relaxed">
                    The SVG will contain the original image as an embedded bitmap — visually identical to the source PNG, works everywhere SVG is supported.
                  </div>
                ) : mode === "high-detail" ? (
                  <div className="rounded-lg bg-blue-50 border border-blue-200 p-3 text-xs text-blue-800 leading-relaxed">
                    Uses 64-color vector tracing. Works best for logos and flat illustrations. Complex photos will produce very large SVG files — use Pixel Perfect for those.
                  </div>
                ) : (
                  <div className="rounded-lg bg-surface-1 border border-border p-3 text-xs text-foreground-muted leading-relaxed">
                    {mode === "flat"
                      ? "16-color tracing — ideal for flat design icons, simple UI graphics, and clip art."
                      : "2-color B&W silhouette — ideal for stamps, watermarks, and single-color logos."}
                  </div>
                )}
            ...
            <div className="glass-panel rounded-[16px] p-4 flex flex-wrap items-center gap-3">
              <div>
                <span className="text-sm font-semibold text-foreground">
                  {results.length} SVG{results.length !== 1 ? "s" : ""} ready
                </span>
              ...
              {results.length > 0 && (
                <button onClick={handleDownloadAll} className="btn btn-primary inline-flex items-center gap-2 text-sm">
                  ...
                  {results.length === 1 ? "Download SVG" : "Download ZIP"}
                </button>
              )}
                ...
                <div key={i} className="glass-panel rounded-[16px] p-5 space-y-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <a
                      href={r.svgUrl}
                      download={r.name}
                      className="btn btn-primary inline-flex items-center gap-1.5 text-xs flex-shrink-0"
                    >
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 rounded-lg px-3 py-2 border" style={{
                      ...
                      <p className="text-[10px] font-medium uppercase tracking-wide" style={{
                        color: r.svgSize < r.originalSize ? "rgb(22 163 74)" : "rgb(37 99 235)"
                      }}>
                      ...
                      <p className="text-sm font-semibold" style={{
                        ...
                        {r.svgSize < r.originalSize
                          ? `-${Math.round((1 - r.svgSize / r.originalSize) * 100)}%`
                          : "∞ scale"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    <p className="text-xs text-foreground-muted mb-2 flex items-center gap-1.5">
                      ...
                      Drag to compare original vs SVG
                    </p>
          ...
          <div className="glass-panel glass-panel-info rounded-[16px] p-6 flex items-start gap-5">
            ...
            <div>
              ...
              <p className="text-foreground-muted text-sm leading-relaxed">
                All conversion happens in your browser. Pixel Perfect mode encodes the image as base64; vector modes trace using imagetracerjs. Nothing is uploaded.
              </p>
            </div>
          </div>
          <div className="glass-panel glass-panel-info rounded-[16px] p-6 flex items-start gap-5">
            ...
            <div>
              ...
              <p className="text-foreground-muted text-sm leading-relaxed">
                Pixel Perfect for exact fidelity, High Detail for full-color logos, Flat for clean shapes, Monochrome for B&W silhouettes — each tuned for its use case.
              </p>
            </div>
          </div>
        </div>
