...
const faqs = [
  {
    q: "What is Shopify's recommended product image size?",
    a: "Shopify recommends 2048×2048 pixels for product images. This ensures images look sharp on Retina displays and high-DPI screens. Shopify automatically creates smaller versions for thumbnails and mobile views. Maximum file size is 20 MB.",
  },
  {
    q: "Should Shopify product images have white backgrounds?",
    a: "White backgrounds are the most common choice for Shopify stores as they create a clean, consistent look. However, Shopify does not mandate white backgrounds. Lifestyle photos, gradient backgrounds, or brand-colored backgrounds also work well depending on your store's aesthetic.",
  },
  {
    q: "What image format works best for Shopify?",
    a: "JPG works best for product photos on Shopify — it offers the best file size to quality ratio for photographic content. Use PNG only for images with transparency (logos, icons). Shopify also supports WebP and AVIF for better compression, but JPG has universal compatibility.",
  },
  {
    q: "Does Shopify compress my images automatically?",
    a: "Yes. Shopify automatically compresses images when they are uploaded and serves them via their CDN. However, uploading pre-optimized images at the right dimensions (2048×2048) ensures Shopify's automatic compression maintains the highest quality.",
  },
];

export default function ShopifyImageOptimizerPage() {
  ...
  const handleResize = useCallback(async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        // Cover mode: scale and center-crop to fill 2048×2048
        ...
        results.push({
          ...
          blob,
          url,
          newName: `${baseName}_shopify_2048x2048.jpg`,
          newSize: blob.size,
          status: "done",
        });
      } catch (err) {
  ...
  const handleDownloadAll = useCallback(async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = "shopify_product_images.zip";
      a.click();
    ...
    <div className="min-h-[calc(100vh-8rem)]">
      <div className="max-w-[1200px] mx-auto px-5 md:px-6 lg:px-8 pt-8 sm:pt-12">
        <ToolHero
          ...
          description="Resize and optimize product images for Shopify to 2048×2048px. White background option, JPG at 85% quality. Retina-ready. Free, private, browser-based."
            ...
            {files.length > 0 && (
              <div className="mt-6 space-y-5 animate-fade-in-up">
                <div className="flex items-center gap-3 p-4 rounded-xl bg-surface-2 border border-border">
                  ...
                  <div>
                    <p className="text-xs font-bold text-foreground">Shopify Optimal: 2048×2048px JPG</p>
                    <p className="text-xs text-foreground-secondary">Retina-ready · Cover crop · 85% quality</p>
                  </div>
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer p-4 rounded-xl border border-border hover:bg-surface-2 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-foreground-secondary">Recommended for a clean, professional Shopify store</p>
                  </div>
                </label>
                  ...
                  >
                    {processing ? (
                      <>
                        ...
                        Optimizing {convertingIndex + 1}/{files.length}...
                      </>
                    ) : (
                      <>
                        <ImageIcon className="w-5 h-5" />
                        Optimize for Shopify ({files.length} image{files.length !== 1 ? "s" : ""})
                      </>
                    )}
                  </button>
                </div>
          ...
          <div className="space-y-4 animate-fade-in-up">
            <div className="glass-panel rounded-[16px] p-4 flex items-center gap-4 flex-wrap">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-green-50 border border-green-100">
                ...
                <span className="text-xs font-semibold text-green-700">{successCount} ready for Shopify</span>
              </div>
                    ...
                    <>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-foreground-secondary">
                          2048×2048px JPG · {formatSize(file.newSize)}
                        </p>
                      </div>
                      <a
                        href={file.url}
            ...
            <div className="flex items-center justify-center gap-3 mt-4">
              <button onClick={handleReset} className="btn btn-secondary inline-flex items-center gap-2">
                <ResetIcon className="w-4 h-4" /> Optimize More Images
              </button>
            </div>
          </div>
        )}
          ...
          <div className="glass-panel glass-panel-info rounded-[16px] p-6 flex items-start gap-5">
            ...
            <div>
              ...
              <p className="text-foreground-muted text-sm leading-relaxed">
                All processing happens locally in your browser. Your product photos never leave your device.
              </p>
            </div>
          </div>
          <div className="glass-panel glass-panel-info rounded-[16px] p-6 flex items-start gap-5">
            ...
            <div>
              <h4 className="font-display font-semibold text-foreground text-sm mb-1.5">Retina-Ready Output</h4>
              <p className="text-foreground-muted text-sm leading-relaxed">
                2048×2048px ensures crisp display on Retina and high-DPI screens. Shopify's CDN handles responsive scaling automatically.
              </p>
            </div>
          </div>
        </div>
